...
static ssize_t ping_regs_show(struct device *pdev,
			      struct device_attribute *attr, char *buf)
{
	struct typec_device_ops *typec_ops = dev_get_drvdata(pdev);
	return typec_ops->ping_regs(buf);
}
...
static DEVICE_ATTR(ping_regs, S_IRUGO, ping_regs_show, NULL);
...
int add_typec_device(struct device *parent, struct typec_device_ops *typec_ops)
{
	...
	if (!typec_ops || !typec_ops->current_detect
	    ...
	    || !typec_ops->port_mode_set || !typec_ops->dump_regs|| !typec_ops->ping_regs) {

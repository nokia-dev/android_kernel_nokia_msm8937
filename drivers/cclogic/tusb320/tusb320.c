...
static ssize_t tusb320_ping_regs(char *buf);
...
static int tusb320_read_reg(u8 reg, u8 * val)
{
	...
	ret = i2c_transfer(di->client->adapter, msgs, ARRAY_SIZE(msgs));
	    if (ret <0){
	        printk(KERN_ERR"%s: ret<0, retry %d\n", __func__, retry_cnt);
	        mdelay(10);
	    }
	    else
	        break;
			 ...
			 const struct i2c_device_id *id)
{
	...
	ret = gpio_direction_output(di->gpio_enb, 1);
	if (ret < 0) {

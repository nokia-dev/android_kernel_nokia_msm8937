...
static int32_t shub_probe_linearacc(struct platform_device *pfdev);
static int32_t shub_remove_linearacc(struct platform_device *pfdev);

#ifdef CONFIG_OF
    static struct of_device_id shub_of_match_tb_linearacc[] = {
        { .compatible = "sharp,shub_linearacc" ,},
        {}
    };
#else
    #define shub_of_match_tb_linearacc NULL
#endif
...
static struct platform_driver shub_linearacc_driver = {
    .probe = shub_probe_linearacc,
    .remove = shub_remove_linearacc,
    .shutdown = NULL,
    .driver = {
        .name = "shub_dev_linearacc",
        .of_match_table = shub_of_match_tb_linearacc,
    },
};

int shub_linearacc_init(void)          /* SHMDS_HUB_3701_01 mod */
{
    int ret;
    
    ret = platform_driver_register(&shub_linearacc_driver);

    return ret;
}

void shub_linearacc_exit(void)         /* SHMDS_HUB_3701_01 mod */
{
    platform_driver_unregister(&shub_linearacc_driver);
}

// late_initcall(shub_linearacc_init); /* SHMDS_HUB_3701_01 del */
// module_exit(shub_linearacc_exit);   /* SHMDS_HUB_3701_01 del */

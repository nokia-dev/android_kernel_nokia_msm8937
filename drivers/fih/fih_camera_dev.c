...
//back
static int fih_camera_dev_backid_show(struct seq_file *m, void *v)
{
	if (cam_info[0].sensor_id)
	{
		pr_info("%s", cam_info[0].sensor_id);
		seq_printf(m, "%s\n", cam_info[0].sensor_id);
	}else
		seq_printf(m, "none\n");
	return 0;
}

static int fih_camera_dev_proc_backid_open(struct inode *inode, struct file *file)
{
	return single_open(file, fih_camera_dev_backid_show, NULL);
};

static struct file_operations backid_file_ops = {
	.owner   = THIS_MODULE,
	.open    = fih_camera_dev_proc_backid_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release
};
//front
static int fih_camera_dev_frontid_show(struct seq_file *m, void *v)
{
	if (cam_info[1].sensor_id)
	{
		pr_info("%s", cam_info[1].sensor_id);
		seq_printf(m, "%s\n", cam_info[1].sensor_id);
	}else
		seq_printf(m, "none\n");
	return 0;
}

static int fih_camera_dev_proc_frontid_open(struct inode *inode, struct file *file)
{
	return single_open(file, fih_camera_dev_frontid_show, NULL);
};

static struct file_operations frontid_file_ops = {
	.owner   = THIS_MODULE,
	.open    = fih_camera_dev_proc_frontid_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release
};
//aux
static int fih_camera_dev_auxid_show(struct seq_file *m, void *v)
{
	if (cam_info[2].sensor_id)
	{
		pr_info("%s", cam_info[2].sensor_id);
		seq_printf(m, "%s\n", cam_info[2].sensor_id);
	}else
		seq_printf(m, "none\n");
	return 0;
}

static int fih_camera_dev_proc_auxid_open(struct inode *inode, struct file *file)
{
	return single_open(file, fih_camera_dev_auxid_show, NULL);
};

static struct file_operations auxid_file_ops = {
	.owner   = THIS_MODULE,
	.open    = fih_camera_dev_proc_auxid_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release
};
...
{
	pr_info("%s : %d", cam_info[0].sensor_name, cam_info[0].position);
...
{
	pr_info("%s : %d", cam_info[1].sensor_name, cam_info[1].position);
...
{
	pr_info("%s : %d", cam_info[2].sensor_name, cam_info[2].position);
...
int fih_camera_dev_init(struct msm_camera_sensor_slave_info *slave_info, char *sensorid)
{
	struct proc_dir_entry *entry = 0;
	char my_proc_name[16];
	int position = slave_info->sensor_init_params.position;
	pr_info("position %d", position);
	...
	switch(position){
		case BACK_CAMERA_B:
			...
			if(sensorid)
				cam_info[0].sensor_id = sensorid;
			entry = proc_create("cam_back_sensorid", 0444, NULL, &backid_file_ops);
			break;
		...
		case FRONT_CAMERA_B:
			...
			if(sensorid)
				cam_info[1].sensor_id = sensorid;
			entry = proc_create("cam_front_sensorid", 0444, NULL, &frontid_file_ops);
			break;
		...
		case AUX_CAMERA_B:
			...
			if(sensorid)
				cam_info[2].sensor_id = sensorid;
			entry = proc_create("cam_aux_sensorid", 0444, NULL, &auxid_file_ops);
			break;

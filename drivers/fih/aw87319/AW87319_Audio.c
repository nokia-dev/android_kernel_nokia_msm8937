...
struct pinctrl_state *pmx_audio_active = NULL;
struct pinctrl_state *pmx_audio_suspend = NULL;

//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// GPIO Control
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
static int AW87319_pinctrl_init(struct i2c_client *client)
{
  int ret = 0;
  aw87319ctrl = devm_pinctrl_get(&client->dev);
  if (IS_ERR(aw87319ctrl)) {
    dev_err(&client->dev, "Cannot find aw87319 pinctrl!");
    ret = PTR_ERR(aw87319ctrl);
    printk("%s devm_pinctrl_get fail!\n", __func__);
  }

  pmx_audio_active = pinctrl_lookup_state(aw87319ctrl, "pmx_audio_active");
  if (IS_ERR(pmx_audio_active)) {
    ret = PTR_ERR(pmx_audio_active);
    printk("%s : pinctrl err, aw87319_rst_high\n", __func__);
  }

  pmx_audio_suspend = pinctrl_lookup_state(aw87319ctrl, "pmx_audio_suspend");
  if (IS_ERR(pmx_audio_suspend)) {
    ret = PTR_ERR(pmx_audio_suspend);
...
{
    printk("%s enter\n", __func__);
    pinctrl_select_state(aw87319ctrl, pmx_audio_suspend);
    msleep(1);
    pinctrl_select_state(aw87319ctrl, pmx_audio_active);
    msleep(10);
...
static ssize_t AW87319_get_reg(struct device* cd,struct device_attribute *attr, char* buf)
{
  ...
  ssize_t len = 0;
  u8 i;
reg_val = I2C_read_reg(0x0);
 printk( "AW87319_get_reg reg 0x0 = 0x%2X,\n ",reg_val);
  for(i=0;i<0x10;i++)
  {
    reg_val = I2C_read_reg(i);
    printk( "AW87319_get_reg reg%2X = 0x%2X,\n ", i,reg_val);
...
static ssize_t AW87319_set_reg(struct device* cd, struct device_attribute *attr, const char* buf, size_t len)
{
  ...
  {
    printk("AW87319_set_reg %x %x\n",databuf[0], databuf[1]);
    I2C_write_reg(databuf[0],databuf[1]);
    msleep(1);
    I2C_write_reg(databuf[0],databuf[1]);
    reg_val = I2C_read_reg(databuf[0]);
    printk( "AW87319_get_reg reg%2X = 0x%2X,\n ", databuf[0],reg_val);
  }
  return len;
}
...
static int AW87319_create_sysfs(struct i2c_client *client)
{
  ...
  //err = sysfs_create_link(dev->parent->parent->parent,dev->parent->kobj,client->name);
  return err;
}
...
static int AW87319_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  ret = AW87319_pinctrl_init(client);
  if (ret != 0) {
     pr_err("[%s] failed to init AW87319 pinctrl.\n", __func__);
     return ret;
   } else {
...
static const struct of_device_id extpa_of_match[] = {
  {.compatible = "awinic,ext_speaker_amp"},
  {},
};
#endif

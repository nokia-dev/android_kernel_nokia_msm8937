...
static int ilitek_i2c_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if(gesture_state == true){
		i2c.suspend_flag = 1;
		DBG("Enter ilitek_i2c_suspend 0x01 0x00, 0x0A 0x01\n");
		buf[0] = 0x01;
		buf[1] = 0x00;
		ret = ilitek_i2c_write_read(i2c.client, buf, 2, 10, buf, 0);
		if(ret < 0){
			DBG(ILITEK_ERROR_LEVEL "%s, 0x01 0x00 set tp suspend err, ret %d\n", __func__, ret);
		}
		msleep(80);
		buf[0] = 0x0A;
		buf[1] = 0x01;
		ret = ilitek_i2c_write_read(i2c.client, buf, 2, 10, buf, 0);
		if(ret < 0){
			DBG(ILITEK_ERROR_LEVEL "%s, 0x0A 0x01 set tp suspend err, ret %d\n", __func__, ret);
		}
		enable_irq_wake(i2c.client->irq);
		//#else
	} else if(gesture_state == false) {
		DBG("Enter ilitek_i2c_suspend 0x01 0x00, 0x02 0x00\n");
  ...
  {
    i2c.stop_polling = 1;
			DBG(ILITEK_DEBUG_LEVEL "%s, stop i2c thread polling\n", __func__);
		}
		...
		ret = ilitek_i2c_write_read(i2c.client, buf, 2, 10, buf, 0);
		if(ret < 0){
			DBG(ILITEK_ERROR_LEVEL "%s, 0x02 0x00 set tp suspend err, ret %d\n", __func__, ret);
		}		
    }
#endif
#endif
...
void touch_upgrade_write(int flag)
{
	...
	{
		CTPM_FW[j] = hex_2_dec(&ilifile_buf[i+2], 2);
		if(j % 16 == 0)
		{
			//printk("0x%06X:", i / 5);
		}
		//printk("0x%02X ", CTPM_FW[j]);
		if(j != 0 && (j+1) % 16 == 0)
		{
			//printk("\n");
		}
	}
	ilifile_len = (( CTPM_FW[26] << 16 ) + ( CTPM_FW[27] << 8 ) + CTPM_FW[28]) + (( CTPM_FW[29] << 16 ) + ( CTPM_FW[30] << 8 ) + CTPM_FW[31]) + 32;
	...
	//if (evdata && evdata->data) {
		if (event == FB_EVENT_BLANK) {
			...
			if (*transition == FB_BLANK_POWERDOWN) {
				printk("F@TOUCH %s FB_BLANK_POWERDOWN\n",__func__);
				ilitek_i2c_suspend(i2c.client, PMSG_SUSPEND);
				//rmi4_data->fb_ready = false;
				i2c.suspend_flag = 1;
			} else if (*transition == FB_BLANK_UNBLANK) {
				printk("F@TOUCH %s FB_BLANK_UNBLANK\n",__func__);
				ilitek_i2c_resume(i2c.client);
				i2c.suspend_flag = 0;
			}
		}

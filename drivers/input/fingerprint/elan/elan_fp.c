...
#include <linux/regulator/consumer.h>	//add by jenny for regulator
#include "elan_fp.h"

#define VERSION_LOG	"2.1.2"
...
static int tb_factory_status = 0;
static DEFINE_MUTEX(elan_factory_mutex);
static DEFINE_MUTEX(elan_suspend_mutex);
static DECLARE_WAIT_QUEUE_HEAD(elan_poll_wq);
static DECLARE_WAIT_QUEUE_HEAD(elan_tb_poll_wq);
static int elan_work_flag = 0;
static int elan_tb_work_flag = 0;
static int suspend_flag = 0; //0:resume, 1:suspend
static int tb_int = 0;
...
struct elan_data  {
    ...
    struct miscdevice       elan_tb_dev;
	struct platform_device	*pdev;
	struct input_dev		*input_dev;
	spinlock_t				irq_lock;
	struct wake_lock		wake_lock;
    struct wake_lock	    hal_wake_lock;
	struct regulator        *pwr_reg;	//add by jenny for regulator
};
...
static long elan_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
        ...
        case ID_IOCTL_SUSPEND_RESUME: //90
            ELAN_DEBUG("[IOCTL] SUSPEND_FLAG = %d", suspend_flag);
            return suspend_flag;

        case ID_IOCTL_TB_INT_FACTORY: //92
            tb_int_tmp = tb_int;
            tb_int = 0;
            return tb_int_tmp;

        case ID_IOCTL_READ_TB_FACTORY_STATUS: //93
            mutex_lock(&elan_factory_mutex);
            ELAN_DEBUG("[IOCTL] READ factory_status = %d", tb_factory_status);
            mutex_unlock(&elan_factory_mutex);
            return tb_factory_status;
            break;

        case ID_IOCTL_WRITE_TB_FACTORY_STATUS: //94
            mutex_lock(&elan_factory_mutex);
            tb_factory_status = (int __user)arg;
            ELAN_DEBUG("[IOCTL] WRITE factory_status = %d\n", tb_factory_status);
            mutex_unlock(&elan_factory_mutex);
            break;
...
static unsigned int elan_tb_poll(struct file *file, poll_table *wait)
{
	int mask=0; 

	wait_event_interruptible(elan_tb_poll_wq, elan_tb_work_flag > 0);
	if(elan_tb_work_flag > 0)
		mask = elan_tb_work_flag;
...
static int elan_tb_open(struct inode *inode, struct file *filp)
{
	struct elan_data *fp = container_of(filp->private_data, struct elan_data, elan_tb_dev);	
	filp->private_data = fp;
	ELAN_DEBUG("%s()\n", __func__);
	return 0;
}

static int elan_tb_close(struct inode *inode, struct file *filp)
{
    ELAN_DEBUG("%s()\n", __func__);
	return 0;
}

static const struct file_operations elan_tb_fops = {
	.owner 			= THIS_MODULE,
	.open 			= elan_tb_open,
	.poll			= elan_tb_poll,
	.release 		= elan_tb_close,
};
...
static irqreturn_t elan_irq_tb_handler(int irq, void *dev_id)
{
    struct elan_data *fp = (struct elan_data *)dev_id;
...
static int elan_setup_cdev(struct elan_data *fp)
{
    ...
    fp->elan_tb_dev.minor = MISC_DYNAMIC_MINOR;
	fp->elan_tb_dev.name = "elan_tb";
	fp->elan_tb_dev.fops = &elan_tb_fops;
	fp->elan_tb_dev.mode = S_IFREG|S_IRWXUGO; 
	if (misc_register(&fp->elan_tb_dev) < 0) {
  		ELAN_DEBUG("TOUCH BUTTON misc_register failed\n");
		return -1;		
	}
  	else {
		ELAN_DEBUG("TOUCH BUTTON misc_register success\n");		
	}
...
{	
    ...
    // Configure TOUCH BUTTON INT GPIO (Input)
	ret = gpio_request(fp->int_tb_gpio, "elan-tb-irq");
	if (ret < 0)
		ELAN_DEBUG("TOUCH BUTTON interrupt pin request gpio failed, ret = %d\n", ret);
	else {
		gpio_direction_input(fp->int_tb_gpio);
		fp->irq_tb = gpio_to_irq(fp->int_tb_gpio);
        if(fp->irq_tb < 0) {
            ELAN_DEBUG("gpio to irq_tb failed, irq_tb = %d", fp->irq_tb);
            ret = -1;
        }
        else
            ELAN_DEBUG("gpio to irq_tb success, irq_tb = %d\n",fp->irq_tb);
	}
...
{
    int rc = 0; //add by jenny for regulator
    ...
    fp->int_tb_gpio = of_get_named_gpio(np, "elan,tb-irq-gpio", 0);
	ELAN_DEBUG("int_tb_gpio = %d\n", fp->int_tb_gpio);
	if (fp->int_tb_gpio < 0)
		return fp->int_tb_gpio;

	/* jenny add for regulator begin */
...
{	
    ...
    ret = request_irq(fp->irq_tb, elan_irq_tb_handler,
                    IRQF_TRIGGER_FALLING | IRQF_ONESHOT, 
                    pdev->dev.driver->name, fp);
    if(ret)
		ELAN_DEBUG("request irq_tb failed, ret = %d\n", ret);
...
static int elan_suspend(struct device *dev)
{
    ...
    mutex_lock(&elan_suspend_mutex);
    suspend_flag = 1;
    mutex_unlock(&elan_suspend_mutex);
...
{
	int ret = 0;
	ELAN_DEBUG("%s() cmdline %s. Start\n", __func__, saved_command_line);

	/* jenny add for cmdline start */
	if(strstr(saved_command_line, "androidboot.fp=elan") == NULL)
	{
		ELAN_DEBUG("%s This FPM vendor is not elan.\n",__func__);
		goto exit;
	}
	/* jenny add for cmdline end */
